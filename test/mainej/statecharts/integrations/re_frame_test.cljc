(ns mainej.statecharts.integrations.re-frame-test
  (:require [re-frame.core :as rf]
            [statecharts.core :as fsm]
            [day8.re-frame.test :refer [run-test-sync]]
            [mainej.statecharts.integrations.re-frame :as fsm.rf]
...
(deftest test-rf-multiple-simultaneous-states
  (let [clock         (fsm.sim/simulated-clock)
    ...
    (run-test-sync
     (rf/dispatch [::fsm.rf/register fsm-path (fsm/machine machine-spec) {:clock clock}])
     ;; request jack's friends
     (rf/dispatch [::fsm.rf/initialize transition-data-jack {:context context-jack}])
     (is (= (get-data [::fsm-state :jack :_state]) :init))
     (is (= (get-data [:users "jack" :friends]) nil))
     (advance-clock 500)
     ;; a moment later request jill's friends
     (rf/dispatch [::fsm.rf/initialize transition-data-jill {:context context-jill}])
     (advance-clock 500)
...
(deftest test-rf-epoch-support
  (let [clock         (fsm.sim/simulated-clock)
    ...
    (run-test-sync
     (rf/dispatch [::fsm.rf/register fsm-path (fsm/machine machine-spec) {:clock clock}])
     (rf/dispatch [::fsm.rf/initialize transition-data {:context context}])
     (is (= (get-data [::fsm-state :jack :_state]) :init))
